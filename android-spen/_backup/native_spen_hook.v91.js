(function () {
    ...
    function nativeGeminiFetch(apiKey, modelName, parts, timeoutMs, genConfig) {
        return new Promise(function (resolve, reject) {
            try {
                ...
                if (typeof window.ServiceHubAndroidSpen.geminiBegin === 'function') {
                    window.ServiceHubAndroidSpen.geminiBegin(id, url);
                    var step = 160000;
                    var i;
                    for (i = 0; i < body.length; i += step) {
                        window.ServiceHubAndroidSpen.geminiChunk(id, body.slice(i, i + step));
                    }
                    window.ServiceHubAndroidSpen.geminiEnd(id);
                } else {
    ...
    function noteCmdBullet() {
        noteFieldApply(function (v, start, end) {
            ...
            var alreadyHere = beforeOnLine.match(/^\s*\u2022\s*$/);
            // Riga gia col pallino (doppio tap / click S Pen): non crearne un secondo.
            if (alreadyHere) {
                return { value: v, pos: start };
            }
    ...
    function runNoteCmd(cmd) {
        cmd = String(cmd || '');
        var now = Date.now();
        var gap = (cmd === 'up' || cmd === 'down' || cmd === 'left' || cmd === 'right' || cmd === 'del') ? 70
            : (cmd === 'dot' || cmd === 'nl' || cmd === 'sp') ? 420 : 180;
    ...
    function noteCmdPanelHtml() {
        return ''
            + '<div class="sh-note-cmd-arrows" aria-label="Frecce cursore">'
            + '<div role="button" tabindex="0" class="sh-note-cmd-btn sh-note-cmd-arrow" data-sh-note-cmd="up" aria-label="Freccia alto">↑</div>'
            + '<div role="button" tabindex="0" class="sh-note-cmd-btn sh-note-cmd-arrow" data-sh-note-cmd="down" aria-label="Freccia basso">↓</div>'
            + '<div role="button" tabindex="0" class="sh-note-cmd-btn sh-note-cmd-arrow" data-sh-note-cmd="left" aria-label="Freccia sinistra">←</div>'
            + '<div role="button" tabindex="0" class="sh-note-cmd-btn sh-note-cmd-arrow" data-sh-note-cmd="right" aria-label="Freccia destra">→</div>'
            + '</div>'
    ...
    function ensureNoteCss() {
        ...
        st.textContent = '#inp-sec-note,#inp-sec-note.note-scheda-textarea{'
            ...
            + 'flex-direction:row;flex-wrap:wrap;justify-content:flex-end;align-items:center;gap:6px;'
            ...
            + '.sh-note-cmd-arrows{display:flex;flex-direction:row;flex-wrap:nowrap;align-items:center;gap:6px;margin-right:4px;}'
            + '.sh-note-cmd-btn.sh-note-cmd-arrow{min-width:40px;padding:0 8px;font-size:20px;background:#0f766e;}'
    ...
    if (typeof window.ensureGeminiKeysNeverLost === 'function') {
        try { window.ensureGeminiKeysNeverLost(); } catch (eKeys) {}
    }
    if (typeof window.loadGeminiKeySettings === 'function') {
        try { window.loadGeminiKeySettings(); } catch (eLoad) {}
    }
    if (typeof window.clearAllGeminiKeyCooldowns === 'function') {
        try { window.clearAllGeminiKeyCooldowns(); } catch (eCd) {}
    }
    function wrapOcrApply(name) {
        if (typeof window[name] !== 'function' || window[name].__shV90) return;
        var orig = window[name];
        ...
        window[name].__shV90 = true;
    }
    wrapOcrApply('applySmartCaptureResults');
    wrapOcrApply('applySmartCaptureDirectly');
    wrapOcrApply('applyDcsReadingsToAllRapportini');
    function dataUrlToOcrFile(dataUrl) {
    ...
    function runWebOcrFromDataUrl(dataUrl) {
        ...
        if (typeof window.runSmartCaptureOnFiles === 'function') {
            Promise.resolve(window.runSmartCaptureOnFiles([dataUrlToOcrFile(dataUrl)]));
        }
    }
    window.__shOcrParts = [];
    window.__shOcrPush = function (chunk) {
        if (chunk) window.__shOcrParts.push(chunk);
    };
    window.__shOcrFinish = function () {
        var b64 = window.__shOcrParts.join('');
        window.__shOcrParts = [];
        runWebOcrFromDataUrl(b64 ? ('data:image/jpeg;base64,' + b64) : '');
    };
    window.__shOcrPull = function () {
        try {
            var br = window.ServiceHubAndroidSpen;
            if (!br || typeof br.ocrJpegLen !== 'function') {
                window.__shNativeOcrError('Foto non disponibile');
                return;
            }
            var n = br.ocrJpegLen() | 0;
            if (n < 80) {
                window.__shNativeOcrError('Foto vuota');
                return;
            }
            var parts = [];
            var i = 0;
            ...
            window.__shNativeOcrError((ePull && ePull.message) || 'Lettura foto fallita');
        }
    };
    window.__shNativeOcrPhoto = function (dataUrl) {
        runWebOcrFromDataUrl(dataUrl);
    };
