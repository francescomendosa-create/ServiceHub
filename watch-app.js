(function () {
  ...
  function formatContatoreDisplay(val) {
    ...
    if (val == null || String(val).trim() === '') return 'â€”';
  ...
  function updateNumpadDisplay() {
    ...
    if (disp) disp.textContent = state.numpadBuffer || 'â€”';
  ...
  function bindSwipeBackGesture() {
    ...
    document.addEventListener('touchstart', function (e) {
      ...
      if (e.target && e.target.closest && e.target.closest('#sw-cnt-numpad-cancel, #sw-cnt-numpad-grid, .sw-numpad-key, #sw-cnt-numpad-display')) {
